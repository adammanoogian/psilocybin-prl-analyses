...
def _make_laplace_idata(
    ...
    var_blocks = [np.full(P, omega2_sd**2), np.full(P, log_beta_sd**2)]
    # Parameters added to the canonical 2-level order after this fixture was
    # written (e.g. choice bias "b"): zero mean, small variance.
    for extra in _PARAM_ORDER_2LEVEL[2:]:
        mode[extra] = np.zeros(P, dtype=np.float64)
        var_blocks.append(np.full(P, 0.05**2))
    cov = np.diag(np.concatenate(var_blocks))
